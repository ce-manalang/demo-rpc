...
async function buildStrictCandidates(items, criteria, excludedIds = [], limit = 3) {
  ...
  // BUT only when price is a range, not a hard upper limit
  const hasPriceRange = criteria.min_price != null && criteria.max_price != null;
  const hasPriceUpperLimit = criteria.min_price == null && criteria.max_price != null;
  const hasPriceLowerLimit = criteria.min_price != null && criteria.max_price == null;
