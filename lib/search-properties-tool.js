...
2. LOCATION HANDLING:
   ...
   **CRITICAL - DO NOT confuse property types with locations:**
   ...
   MULTIPLE LOCATIONS (CRITICAL for comparison queries):
   - If query mentions 2+ locations for comparison: "one from Cavite one from Taguig", "compare Makati vs BGC"
   - Set filter_location to COMMA-SEPARATED list: "Cavite, Taguig"
   - The search will find properties matching ANY of these locations (OR logic)
   - Examples:
   ...
   - If query mentions "compare" or "vs", set requested_count to match number of locations (e.g., 2 for 2 locations)
...
7. COUNT EXTRACTION (DO THIS FIRST):
   ...
   - **COMPARISON QUERIES**: If query asks to compare multiple locations:
     • "pick 2 properties one from cavite one from taguig" → requested_count: 2
     • "compare Makati vs BGC" → requested_count: 2 (one per location)
     • "show properties in Manila, Quezon City, and Pasig" → requested_count: 3 (one per location)
