...
async function rankByEmbeddings(candidates, criteria) {
  ...
  const scored = candidates.map((candidate) => {
    ...
    if (similarity > 0.8) {
      reasonParts.push('Excellent match for your search');
    } else if (similarity > 0.6) {
      reasonParts.push('Good match');
    } else {
      reasonParts.push('Partial match');
    }
    
    if (params.filter_location && candidate.locationName?.includes(params.filter_location)) {
      reasonParts.push(`in ${params.filter_location}`);
    } else if (candidate.distanceKm) {
      reasonParts.push(`${candidate.distanceKm}km away`);
    }
    ...
    if (params.max_price && candidate.price) {
      ...
      if (ratio <= 0.8) reasonParts.push('well within budget');
