...
async function buildStrictCandidates(items, criteria, excludedIds = []) {
    ...
    // Cap K for reranker cost control (reduced from 30 to 15 for performance)
    const K = 15;
    ...
    const slim = capped.map((p) => {
        ...
        const unitTypeSummary = Array.isArray(p.unitConfiguration)
            ...
            : [];
        // Limit amenitiesTags to first 15 items to reduce payload size
        ...
        const amenitiesTags = amenitiesTagsFull.slice(0, 15);
...
7. COUNT EXTRACTION (DO THIS FIRST):
   ...
   - **ONLY set requested_count: 1** when query asks for "THE lowest" or "THE cheapest" WITHOUT any explicit number
     ...
     • "Top 5 lowest prices" → requested_count: 5 (explicit count takes priority)
     • "Show me the cheapest ones" → requested_count: null (plural, no specific count)
