...
async function buildStrictCandidates(items, criteria, excludedIds = []) {
    ...
    const propertyMeetsBedrooms = (prop) => {
        if (criteria.min_bedrooms == null && criteria.max_bedrooms == null) return true;
        
        // Check property-level bedrooms
        const propBedrooms = Number.isFinite(Number(prop.bedrooms)) ? Number(prop.bedrooms) : null;
        
        // Check unit-level bedrooms
        const unitBedrooms = Array.isArray(prop.unitConfiguration)
            ? prop.unitConfiguration.map(u => Number(u.bedrooms)).filter(Number.isFinite)
            : [];
    ...
    if (criteria.sort_by === 'price_asc') {
        deduped.sort((a, b) => {
            const unitPricesA = Array.isArray(a.unitConfiguration)
                ? a.unitConfiguration.map(u => Number(u.configPrice)).filter(Number.isFinite)
                : [];
            const unitPricesB = Array.isArray(b.unitConfiguration)
                ? b.unitConfiguration.map(u => Number(u.configPrice)).filter(Number.isFinite)
                : [];
            const minPriceA = unitPricesA.length ? Math.min(...unitPricesA) : (a.price || Infinity);
            const minPriceB = unitPricesB.length ? Math.min(...unitPricesB) : (b.price || Infinity);
        ...
        deduped.sort((a, b) => {
            ...
            const unitPricesB = Array.isArray(b.unitConfiguration)
                ? b.unitConfiguration.map(u => Number(u.configPrice)).filter(Number.isFinite)
                : [];
            const maxPriceA = unitPricesA.length ? Math.max(...unitPricesA) : (a.price || 0);
            const maxPriceB = unitPricesB.length ? Math.max(...unitPricesB) : (b.price || 0);
            return maxPriceB - maxPriceA;
        });
    }

    // Cap K for reranker cost control (reduced to 12 to avoid payload truncation)
    // Now we cap AFTER sorting, so we get the actual top 12 cheapest/most expensive
    const K = 12;
    ...
    const slim = capped.map((p) => {
        ...
        return {
            ...
            developer: p.developer?.fullName || p.project?.developer?.shortName || null,
            unitTypeSummary,
            amenitiesTags,
...
Only return valid JSON—no extra commentary.`;
        ...
        if (!parsed.apiSearchParams?.filter_location && parsed.apiSearchParams?.query) {
            ...
            const genericWords = ['property', 'properties', 'condo', 'condos', 'house', 'houses', 
                                 ...
                                 'studio', 'bachelor pad', 'bachelor\'s pad', 'loft', 'penthouse',
                                 'townhouse', 'duplex', 'bi-level'];
            const isGenericQuery = genericWords.some(word => query.toLowerCase() === word || query.toLowerCase().includes(word));
...
- Keep reasons very short (≤120 chars).`;
        ...
        try {
            ...
            if (!Array.isArray(parsed.orderedIds)) {
                console.log('[Rerank Properties Tool] Invalid orderedIds, using fallback');
                return JSON.stringify({ 
            ...
            console.log('[Rerank Properties Tool] JSON parse error, using fallback:', parseError.message);
