...
async function buildStrictCandidates(items, criteria, excludedIds = [], limit = 3) {
  ...
  const propertyMeetsType = (prop) => {
    ...
    const hasBiLevel = /\b(bi[- ]?level|bi[- ]?level unit)s?\b/i.test(queryText);
    const hasLoft = /\b(loft|loft unit)s?\b/i.test(queryText);
    const hasPenthouse = /\b(penthouse|penthouse unit)s?\b/i.test(queryText);
    const hasStudio = /\b(studio|bachelor pad|bachelor's pad)s?\b/i.test(queryText);
  ...
  // Fifth pass: Fallback for exact bathroom count queries
  // If user requested exact bathroom count (e.g., "10 bathrooms") and no results found,
  // remove bathroom filter and return properties with highest bathroom count
  // NOTE: This runs BEFORE semantic candidate generation, so if this populates candidates,
  // semanticCandidates won't be generated (candidates.length > 0 prevents semantic fallback)
  const isExactBathroomMatch = criteria.min_bathrooms != null && 
                                criteria.max_bathrooms != null && 
                                criteria.min_bathrooms === criteria.max_bathrooms;
  
  if (isExactBathroomMatch && candidates.length === 0) {
    console.log(`[Build Candidates] No exact bathroom match (${criteria.min_bathrooms} bathrooms), falling back to highest bathroom count`);
    
    // Collect all properties that meet other criteria (except bathroom)
    ...
    for (const it of items) {
      ...
      if (!propertyMeetsType(it)) continue;
      if (!propertyMeetsBedrooms(it)) continue;
      // Skip bathroom check - this is the fallback
      if (!propertyMeetsDeveloper(it)) continue;
      if (!propertyMeetsProject(it)) continue;
      if (!propertyHasRequiredAmenities(it)) continue;
      if (locations && !propertyTextContainsAnyLocation(it, locations)) continue;
      
      fallbackCandidates.push(it);
    }
    
    // Sort by bathroom count descending (highest first)
    fallbackCandidates.sort((a, b) => {
      const getMaxBathrooms = (prop) => {
        const propBathrooms = Number.isFinite(Number(prop.bathrooms)) ? Number(prop.bathrooms) : 0;
        const unitBathrooms = Array.isArray(prop.unitConfiguration)
          ? prop.unitConfiguration
              .map((u) => Number(u.bathrooms))
              .filter(Number.isFinite)
          : [];
        const maxUnitBathrooms = unitBathrooms.length > 0 ? Math.max(...unitBathrooms) : 0;
        return Math.max(propBathrooms, maxUnitBathrooms);
      };
      
      const maxA = getMaxBathrooms(a);
      const maxB = getMaxBathrooms(b);
      return maxB - maxA; // Descending order
    });
    
    // Use fallback candidates (respect limit)
    candidates.push(...fallbackCandidates.slice(0, limit));
  }
...
Example: { "query": "...", "filter_location": "Makati", "max_price": 5000000, "excludedPropertyIds": ["id1"] }`,
  ...
  execute: async ({ criteria }) => {
    ...
    // This includes bathroom fallback logic for exact bathroom count queries
    ...
    // NOTE: If bathroom fallback ran, candidates will be populated here, preventing semanticCandidates generation
    const out = {
      candidates,
