...
- **If count ≥ 4**, ALWAYS call rerank_properties with:
  - The tool automatically chooses the best ranking method:
    * Location-specific queries (e.g., "in Makati", "near BGC") → Always use LLM for accurate location understanding
    * Generic queries without location (e.g., "2-bedroom under 5M") with 4-5 results → Fast template-based ranking
    * Complex queries or >5 results → LLM-based ranking
