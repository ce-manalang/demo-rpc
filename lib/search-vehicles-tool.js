...
async function buildStrictCandidates(items, criteria, excludedIds = [], limit = 3) {
  ...
  const vehicleMeetsCategory = (vehicle) => {
    ...
    return categoryFilters.some((filter) => vcategory.includes(filter) || filter.includes(vcategory));
  };
  ...
  for (const it of items) {
    ...
    if (!vehicleMeetsPrice(it)) continue;
    if (!vehicleMeetsCategory(it)) continue;
    if (!vehicleMeetsFuelType(it)) continue;
    if (!vehicleMeetsSeating(it)) continue;
    if (!vehicleMeetsDistributor(it)) continue;
    if (!vehicleMeetsBrand(it)) continue;
    if (!vehicleMeetsModel(it)) continue;
    if (!vehicleHasRequiredFeatures(it)) continue;
    if (locations && !vehicleTextContainsAnyLocation(it, locations)) continue;
