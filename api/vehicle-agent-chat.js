/**
 * Vehicle Agent Chat Endpoint
 * Handles Sakai Car Dealership Assistant conversations using OpenAI Agents SDK
 */

import { run } from '@openai/agents';
import { OpenAI } from 'openai';
import { sakaiAgent } from '../lib/sakai-agent.js';
...
export default async function handler(req, res) {
  ...
  console.log('[Vehicle Agent Chat] Request received');
  ...
  try {
    ...
    if (isNewChat) {
      ...
      conversationId = null;
      console.log('[Vehicle Agent Chat] New chat requested, cleared cached conversationId for client', clientId);
    }
    ...
    if (!conversationId) {
      ...
      console.log('[Vehicle Agent Chat] Created new conversationId for client', clientId, conversationId);
    }

    console.log('[Vehicle Agent Chat] Running agent with conversation input');
    console.log('[Vehicle Agent Chat] Input length:', conversationInput.length);
    console.log('[Vehicle Agent Chat] Using conversationId:', conversationId);

    // Run the sakai agent
    const result = await run(sakaiAgent, conversationInput, { conversationId });

    const responseText = result.finalOutput || '';
    console.log('[Vehicle Agent Chat] Agent completed. Response length:', responseText.length);
    ...
    console.error('[Vehicle Agent Chat] Error:', error);
    res.status(500).json({ 
